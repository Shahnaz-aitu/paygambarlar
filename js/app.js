(function(){
const app=document.getElementById('app');const nav=document.getElementById('prophetNav');let activeTab='story';
const preview={adam:['🌿','☁️','💧'],idris:['🧵','🪡','〰'],nuh:['⛵','🌧️','🌊'],hud:['🏛️','🌾','💨'],salih:['⛰️','🐪','☀️'],lut:['🏠','✦','🌙'],ibrahim:['🔥','✦','🔥'],ismail:['🕋','☀️','💧'],ishaq:['⛺','🌿','☀️'],yaqub:['📖','🪔','🪟'],yusuf:['🪣','🕳️','☀️'],shuayb:['🐪','📦','〰'],ayyub:['🪟','🌱','☀️'],dhulkifl:['⚖️','🏛️','✦'],musa:['△','☀️','〰'],harun:['✦','◼','☀️'],dawud:['🛡️','⚔️','✦'],sulayman:['🏛️','🐦','⛲'],ilyas:['⛰️','⚡','☁️'],alyasa:['🌿','💧','⛰️'],yunus:['🐋','●','💧'],zakariya:['🌳','🍃','☀️'],yahya:['☾','✦','✧'],isa:['🌿','💧','☀️'],muhammad:['🌹','🌿','🌸']};
function esc(s=''){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function refParts(ref){const [s,a]=ref.split(':');return{surah:Number(s),ayah:a}}
function expandRef(ref){const {surah,ayah}=refParts(ref);if(!ayah.includes('-'))return[`${surah}:${ayah}`];const [a,b]=ayah.split('-').map(Number);return Array.from({length:b-a+1},(_,i)=>`${surah}:${a+i}`)}
function arabicFor(ref){return expandRef(ref).map(k=>window.quranArabic[k]||'').filter(Boolean).join(' ۝ ')}
function quranUrl(ref){const {surah,ayah}=refParts(ref);return`https://quran.com/${surah}/${ayah.split('-')[0]}`}
function refLabel(ref){const {surah}=refParts(ref);return`${window.surahNames[surah]||(t('surah')+' '+surah)} ${ref}`}
function renderNav(){nav.innerHTML=window.prophetsData.map(p=>`<button class="nav-item ${location.hash==='#'+p.id?'active':''}" data-prophet-link="${p.id}"><span class="nav-num">${String(p.number).padStart(2,'0')}</span><span>${esc(p.name[currentLang])}</span></button>`).join('')}
function ayahMeaningFor(ref,fallback=''){const item=window.quranMeaning?.[ref];return item?.[currentLang]||fallback||t('ayahMeaningGeneric')}
...
function rerenderCurrent(){const id=location.hash.slice(1);if(id&&window.prophetsData.some(p=>p.id===id))renderProphet(id);else renderHome()}
document.addEventListener('click',e=>{const tab=e.target.closest('[data-tab]');if(tab){activeTab=tab.dataset.tab;const id=location.hash.slice(1);renderProphet(id);return}const tog=e.target.closest('[data-ayah-toggle]');if(tog){tog.closest('.ayah-block').classList.toggle('open');return}const s=e.target.closest('[data-scroll-ayah]');if(s){const el=document.getElementById(s.dataset.scrollAyah);el?.scrollIntoView({behavior:'smooth',block:'start'});setTimeout(()=>el?.querySelector('.ayah-block')?.classList.add('open'),280)}const r=e.target.closest('[data-open-ref]');if(r){window.open(quranUrl(r.dataset.openRef),'_blank','noopener')}});
document.addEventListener('keydown',e=>{const c=e.target.closest?.('.prophet-card');if(c&&(e.key==='Enter'||e.key===' ')){e.preventDefault();openProphet(c.dataset.prophetLink)}});
window.addEventListener('hashchange',()=>{activeTab='story';rerenderCurrent();scrollTo({top:0,behavior:'smooth'})});window.addEventListener('languagechange',rerenderCurrent);
window.addEventListener('scroll',()=>{const d=document.documentElement,max=d.scrollHeight-innerHeight;document.documentElement.style.setProperty('--reading',max>0?`${Math.min(100,scrollY/max*100)}%`:'0%')},{passive:true});
applyLanguage(currentLang);rerenderCurrent();
})();
